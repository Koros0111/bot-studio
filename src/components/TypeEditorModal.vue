...
import { defaultForNode, parseTypeNode, seedForNode, serializeNode, type TypeNode } from "@/lib/typeSchema";
...
function primeForm() {
  ...
  formState.value =
    parsed !== undefined ? seedForNode(rootNode.value, props.schema, parsed) : defaultForNode(rootNode.value, props.schema);
}
...
<template>
  <Teleport to="body">
    <Transition name="modal-pop">
      <div v-if="open" class="fixed inset-0 z-50 grid place-items-center p-4" role="presentation">
        <div class="absolute inset-0 bg-ink-950/50 backdrop-blur-sm dark:bg-black/70" @click="cancel" />
        <div ref="dialogRef"
          class="panel relative flex max-h-[85vh] w-full max-w-2xl flex-col overflow-hidden rounded-xl" role="dialog"
          aria-modal="true" :aria-labelledby="titleId" tabindex="-1" @keydown="onKeydown">
          ...
          <div class="absolute inset-x-0 top-0 h-1 bg-gradient-to-r from-signal-blue via-signal-cyan to-signal-blueBright" />

          <div class="relative overflow-hidden border-b border-ink-950/[0.08] px-5 py-4 dark:border-paper-50/[0.08]">
            <div
            ...
            <div class="relative flex items-center justify-between gap-3">
              <div class="flex min-w-0 flex-1 items-center gap-3">
                ...
                <div class="min-w-0 flex-1">
                  <h3 :id="titleId" class="truncate text-lg font-black">{{ displayName(parameter.name) }} builder</h3>
                  <p
                    class="mt-1 block w-fit max-w-full truncate rounded-md bg-paper-200 px-2 py-0.5 font-mono text-[0.7rem] font-bold text-ink-700 dark:bg-navy-700 dark:text-paper-300">
                    {{ parameter.type }}
                  </p>
                </div>
              </div>
              <button type="button" class="icon-button shrink-0" title="Close" aria-label="Close" @click="cancel">
                <X class="h-4 w-4" />
              </button>
            </div>
          </div>
          ...
          <div class="min-h-0 flex-1 overflow-y-auto px-5 py-4">
            <TypeFieldEditor v-if="formState !== null" :node="rootNode" :schema="schema" :depth="0" :visited="[]"
              v-model="formState" />
          ...
          <div
            class="flex items-center justify-between gap-3 border-t border-ink-950/[0.08] bg-paper-100/60 px-5 py-4 dark:border-paper-50/[0.08] dark:bg-navy-900/40">
