...
<template>
  <Teleport to="body">
    <div v-if="open" class="fixed inset-0 z-50 grid place-items-center p-4 sm:p-6" role="presentation">
      <div class="absolute inset-0 bg-ink-950/50 backdrop-blur-sm dark:bg-black/70" @click="close" />
      <div ref="dialogRef"
        class="panel relative flex h-[calc(100vh-2rem)] w-full max-w-6xl flex-col overflow-hidden rounded-xl sm:h-[calc(100vh-3rem)]"
        role="dialog" aria-modal="true" :aria-labelledby="titleId" tabindex="-1" @keydown="onKeydown">
        <div
          ...
          <div class="flex items-center gap-2">
            <slot name="actions" />
            <button type="button" class="icon-button" title="Close" aria-label="Close" @click="close">
              <X class="h-4 w-4" />
            </button>
          </div>
        </div>
