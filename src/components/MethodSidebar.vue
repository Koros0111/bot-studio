...
// that's safe under the GitHub Pages project-subpath deployment. We only
// intercept plain left-clicks (no modifier keys) to do SPA navigation via
// pushState; anything else (middle click, ctrl/cmd/shift-click) is left to
// native browser behavior so "open in new tab" etc. keep working.
...
<template>
  <aside class="panel self-start overflow-hidden rounded-xl p-4">
    <div
      ...
      <input v-model="search" class="min-w-0 flex-1 bg-transparent text-sm outline-none" type="search"
    ...
    <div
      ...
      <select v-model="category" class="min-w-0 flex-1 bg-transparent text-sm font-bold outline-none"
        aria-label="Category">
        <option v-for="item in categories" :key="item">{{ item }}</option>
      </select>
    </div>

    <div class="mt-4 grid h-[20rem] auto-rows-min content-start gap-2 overflow-y-auto pr-1 lg:h-[calc(100vh-14.5rem)]">
      <div v-if="loading" class="empty-pane">Loading Telegram schema...</div>
      <a v-for="method in visibleMethods" :key="method.name" :href="method.name"
        class="group min-w-0 rounded-lg border p-3 text-left transition hover:-translate-y-0.5" :class="selectedName === method.name
          ...
          " :aria-current="selectedName === method.name ? 'page' : undefined" @click="onMethodClick($event, method)">
