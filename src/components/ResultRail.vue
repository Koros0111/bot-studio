...
const statusClass = computed(() => {
  ...
  if (props.response.status === "loading") return "bg-signal-blue/15 text-ink-950 dark:bg-signal-blueDark/20 dark:text-paper-50";
...
const requestMaximizeBtn = ref<HTMLButtonElement | null>(null);
const responseMaximizeBtn = ref<HTMLButtonElement | null>(null);
...
const requestBackdropEl = ref<HTMLElement | null>(null);
const requestFullscreenBackdropEl = ref<HTMLElement | null>(null);
  ...
  <aside class="grid min-w-0 content-start gap-4 xl:block xl:space-y-4">
    <section class="panel min-w-0 rounded-xl p-4">
      <div class="mb-3 flex items-center justify-between gap-3">
        ...
        <div class="flex items-center gap-2">
          <button ref="requestMaximizeBtn" class="icon-button" type="button" title="Maximize request"
            aria-label="Maximize request" @click="requestFullscreen = true">
      ...
      <div class="code-pane json-editor-shell min-h-48 w-full resize-y">
        <pre ref="requestBackdropEl" class="json-editor-box json-editor-backdrop" aria-hidden="true" v-html="highlightedRequest"></pre>
        <textarea v-model="requestJson" class="json-editor-box json-editor-input" spellcheck="false"
          aria-label="Request JSON" @scroll="syncRequestScroll" />
      </div>
      <p v-if="requestError"
    ...
    <section class="panel min-w-0 rounded-xl p-4">
      <div class="mb-3 flex items-center justify-between gap-3">
        ...
        <div class="flex items-center gap-2">
          ...
          <button ref="responseMaximizeBtn" class="icon-button" type="button" title="Maximize response"
            aria-label="Maximize response" :disabled="!responseText" @click="responseFullscreen = true">
            <Maximize2 class="h-4 w-4" />
          </button>
    ...
    <FullscreenModal v-model:open="requestFullscreen" title="Request JSON">
      <div class="code-pane json-editor-shell min-h-0 max-h-none w-full flex-1 text-base leading-7">
        <pre ref="requestFullscreenBackdropEl" class="json-editor-box json-editor-backdrop"
          aria-hidden="true" v-html="highlightedRequest"></pre>
        <textarea v-model="requestJson" class="json-editor-box json-editor-input" spellcheck="false"
          aria-label="Request JSON (fullscreen)" @scroll="syncRequestFullscreenScroll" />
      </div>
      <p v-if="requestError"
    ...
    <FullscreenModal v-model:open="responseFullscreen" title="Response JSON">
      ...
      <pre v-if="responseText"
        class="code-pane min-h-0 max-h-none w-full flex-1 text-base leading-7" v-html="highlightedResponse"></pre>
      <div v-else class="empty-pane">Telegram response will appear here as syntax-highlighted JSON.</div>
    </FullscreenModal>
  </aside>
</template>
