...
const builderTriggerRef = ref<HTMLButtonElement | null>(null);
...
<template>
  <label
    ...
    <ExpandableText tag="blockquote"
      ...
      :text="parameter.description" :lines="2" />
    ...
    <div v-else-if="kind === 'file'" class="mt-4">
      <div
        ...
        <input v-model="fileState.text"
          ...
          type="text" :placeholder="fileState.file ? `${fileState.file.name}` : 'file_id or https://...'"
        ...
        <button v-if="fileState.mode === 'file'" class="icon-button m-1" type="button" title="Remove file"
    ...
    <div v-else-if="kind === 'textarea' || kind === 'json'" class="relative mt-4">
      <textarea v-model="textValue" class="control min-h-32 resize-y py-3" :class="{ 'pr-11': showBuilder }"
        :placeholder="kind === 'json' ? '{ }' : parameter.name + ':'" />
      <button v-if="showBuilder" ref="builderTriggerRef" type="button"
        class="icon-button absolute right-2 top-2 h-7 w-7 bg-paper-50/95 backdrop-blur dark:bg-navy-800/95"
        title="Open visual builder" aria-label="Open visual builder" @click="builderOpen = true">
        <Wand2 class="h-3.5 w-3.5" />
      </button>
      <TypeEditorModal v-if="showBuilder && schema" v-model="textValue" v-model:open="builderOpen"
        :parameter="parameter" :schema="schema" />
    </div>

    <input v-else v-model="textValue" class="control mt-4 h-11" :type="kind === 'number' ? 'number' : 'text'"
