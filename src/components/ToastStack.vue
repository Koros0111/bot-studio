...
function toneClasses(tone: Notice["tone"]) {
  if (tone === "success") return "border-signal-lime/30 bg-signal-lime/10 text-ink-950 dark:text-paper-50";
  if (tone === "error") return "border-signal-red/30 bg-signal-red/10 text-ink-950 dark:text-paper-50";
  if (tone === "warning") return "border-signal-amber/40 bg-signal-amber/10 text-ink-950 dark:text-paper-50";
...
<template>
  <Teleport to="body">
    <div class="fixed right-3 top-3 z-50 grid w-[min(24rem,calc(100vw-1.5rem))] gap-2">
      <TransitionGroup enter-active-class="transition duration-200" enter-from-class="translate-y-2 opacity-0"
