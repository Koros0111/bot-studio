...
const themeLabel = computed(() => (theme.value === "dark" ? "Dark" : theme.value === "light" ? "Light" : "System"));
...
<template>
  <div class="app-shell">
    ...
    <header
      ...
      <div class="mx-auto flex w-full max-w-[1540px] flex-row items-center justify-between gap-3">
        ...
        <div class="flex flex-wrap items-center gap-2">
          <button type="button" class="group/theme icon-button relative"
            :aria-label="`Theme: ${themeLabel}. Click to switch.`" :title="`Theme: ${themeLabel}. Click to switch.`"
            @click="cycleTheme">
          ...
          <a class="icon-button" href="https://github.com/telefunction/bot-studio" target="_blank" rel="noreferrer"
            title="telefunction" aria-label="View Bot Studio source on GitHub">
    ...
    <main
      ...
      <MethodSidebar v-model:search="search" v-model:category="category" :categories="categories"
        :methods="filteredMethods" :selected-name="selected?.name" :loading="loadingSchema" @select="selectMethod" />
        ...
        <div class="border-b border-ink-950/[0.08] pb-5 dark:border-paper-50/[0.08]">
          <div class="flex min-w-0 items-start justify-between gap-3">
            <div class="min-w-0">
              ...
              <ExpandableText tag="p" class="mt-3 max-w-4xl text-sm leading-7 text-ink-700 dark:text-paper-300" :text="selected?.description ||
                'Search or browse the generated Telegram Bot API schema, then choose a method to build a request.'
                " :lines="3" />
            </div>
            <a class="icon-button"
              :href="selected?.officialUrl || schema?.source || 'https://core.telegram.org/bots/api'" target="_blank"
              rel="noreferrer" title="Official docs">
              ↗
            </a>
          </div>
        </div>
        ...
        <form class="mt-5" @submit.prevent="submit">
          ...
          <div class="mt-5 flex flex-col gap-2 sm:flex-row">
            <button
              ...
              type="submit" :disabled="!selected || response.status === 'loading'">
              <Send class="h-4 w-4" />
      ...
      <ResultRail v-model:request-json="requestJson" :request-error="requestJsonError" :response="response"
    ...
    <footer
      ...
      <div>
        Powered by
        <a class="font-black text-signal-blueHover hover:underline dark:text-signal-blueDark dark:hover:text-signal-blueBright"
          href="https://github.com/telefunction" target="_blank" rel="noreferrer" aria-label="Telefunction on GitHub">
          @Telefunction
        </a>
      </div>
